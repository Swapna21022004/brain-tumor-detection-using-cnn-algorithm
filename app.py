...
# --- PDF Header Branding (S in white circle + BTD in blue) ---
    ...
    c.drawString(400, height - 80, f"Date: {date}")

    # Header Text (below logo)
    c.setFont("Helvetica-Bold", 18)
    c.setFillColor(colors.blue)
    c.drawString(200, height - 110, "Brain Tumor Detection Report")


    c.setFillColor(colors.black)
    c.setFont("Helvetica", 12)
    
    c.drawString(60, height - 160, f"Scan ID: {scan_id}")
    c.drawString(60, height - 180, f"Patient Name: {name}")
    c.drawString(60, height - 200, f"Age: {age}")
    c.drawString(60, height - 220, f"Gender: {gender}")
    c.drawString(60, height - 240, f"Contact: {contact}")

    c.setFont("Helvetica-Bold", 14)
    c.setFillColor(colors.HexColor("#d32f2f") if result == "Yes Brain Tumor" else colors.HexColor("#388e3c"))
    c.drawString(150, height - 300, f"Prediction Result: {result}")

    if "No Brain Tumor" in result:
        result_text = "MRI scan of the brain does not reveal any abnormal mass." 
        " No signs of neoplasm are detected. Normal anatomical structures are preserved."
    else:
        result_text = "MRI scan of the brain reveals a space-occupying lesion," \
        " indicating presence of a tumor." \
        " Immediate neurological consultation is advised."

    c.setFillColor(colors.black)
    c.drawString(60, height - 340, f"MRI Scan Diagnosis: {result_text}")

    img_path = os.path.join(UPLOAD_FOLDER, filename)
    try:
        c.drawImage(img_path, 200, height - 600, width=200, height=200, preserveAspectRatio=True, mask='auto')
    except:
        pass
